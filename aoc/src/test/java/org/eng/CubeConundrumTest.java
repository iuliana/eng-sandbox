...
import static org.eng.CubeConundrum.TOTAL;
...
public class CubeConundrumTest {
    ...
    void testSampleZero() throws IOException {
        testTheThing("cube/cube-00");
    }

    @Test
    void testOtherSampleZero() throws IOException {
        testTheOtherThing("cube/cube-00");
    }

    @Test
    void testSampleOne() throws IOException {
        testTheThing("cube/cube-01");
    }

    @Test
    void testOtherSampleOne() throws IOException {
        testTheOtherThing("cube/cube-01");
    }


    void testTheThing(String filename) throws IOException {
        int sum = 0;
        try(BufferedReader br = InReader.getReader(filename + ".in", this.getClass())) {
            String line;
        ...
        out.println("sum= " + sum);
        var output  = Integer.parseInt(InReader.lines(filename + ".out", this.getClass()).toList().getFirst());
        ...
        try(BufferedReader br = InReader.getReader(filename + ".in", this.getClass())) {
            ...
            while ( (line = br.readLine()) != null) {
                ...
                if(g != null) {
                    ...
                    //out.println("g.id()} = \{few");
                    sum += few;
                }
            }
        }
        out.println("sum= " + sum);
        var output  = Integer.parseInt(InReader.lines(filename + "-2.out", this.getClass()).toList().getFirst());
        assertEquals(output, sum);
    }
}
