...
public class BasicLoopIITest {
    private static final Logger LOG = LoggerFactory.getLogger(BasicLoopIITest.class);

    @Test
    void testSampleZero() throws IOException {
        testTheThing("simple/basic-loop-00");
    }

    @Test
    void testSampleOne() throws IOException {
        testTheThing("simple/basic-loop-01");
    }
    ...
    void testTheThing(String filename) throws IOException {
        ...
        var output  = InReader.lines(filename + ".out", this.getClass()).toList();

        var check = true;
        for (int i = 0; i < output.size(); i++) {
            out.println(output.get(i) +" <> " + result.get(i));
