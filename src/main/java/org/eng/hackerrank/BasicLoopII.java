...
public class BasicLoopII {
    ...
    public static Function<String,String> COMPUTE =  line -> {
        ...
        if(a<0 || a>50) {
            throw new IllegalArgumentException(STR."A: \{a} outside interval!");
        }

        if(b<0 || b>50) {
            throw new IllegalArgumentException(STR."B: \{b} outside interval!");
        }

        if(n<1 || n>15) {
            throw new IllegalArgumentException(STR."N: \{n} outside interval!");
        }
        ...
        for (int i = 0; i < n ; i++) {
            ...
            out.append(i == n - 1 ? STR."\{res}" : STR."\{res} ");
        }
      return out.toString();
    };
